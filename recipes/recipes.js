...
function tagsTemplate(tags){
	//returns tags from a list of tags as buttons in an html file
...
function ratingTemplate(rating){
	...
	while(i<=5){
		i++;
		if (i<=rating){
			//adds a full star if the rating is greater than or equal to the loop variable
			html += `<span aria-hidden="true" class="star">⭐</span>`;
		}
		else{
			//adds an empty star if the rating is less than the loop variable
...
function recipeTemplate(recipe){
	return `<img id="food" src=${recipe.image} alt="${recipe.name}">
            <div id="recipe-info">
                ${tagsTemplate(recipe.tags)}    
                <h2 id="food-name">${recipe.name}</h2>
...
function recipeSearch(){
	let uQuery = query.value;
	let filteredRecipes = recipes.filter((recipe)=>{
			return(
			recipe.name.toLowerCase().includes(uQuery.toLowerCase())||
			recipe.description.toLowerCase().includes(uQuery.toLowerCase())||
			recipe.tags.find((tag)=>tag.toLowerCase().includes(uQuery.toLowerCase()))
			)})
...
btn.addEventListener('click', recipeSearch);
	function initRender(){
		renderRecipe(recipes[randNum]);
	}
	initRender();